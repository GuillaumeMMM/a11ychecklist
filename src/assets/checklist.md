# 1. Le contraste entre la couleur du texte et la couleur de l'arrière-plan est suffisamment élevé

## Quels critères concernés ?
...
Si le contraste n'est pas suffisamment élevé, le texte est difficile à lire.

Cette règle s'applique aussi aux composants d'interface non textuels qui sont porteurs d'information (par exemple une pastille d'alerte ou le SVG de l'icône ouvrant le menu).

## Comment vérifier ?

Utiliser un vérificateur de contrastes en ligne comme celui de [coolors](https://coolors.co/contrast-checker/112a46-acc8e5), utiliser un [plugin Figma](https://cssauthor.com/best-figma-contrast-checker-plugins/) ou utiliser une extension web comme [WCAG color contrast checker](https://chromewebstore.google.com/detail/wcag-color-contrast-check/plnahcmalebffmaghcpcmpaciebdhgdf) qui vérifie toute la page d'un coup.
...
### Calcul du contraste trop rigide

Le contraste est calculé de la manière suivante :
...
cf https://accessibilite.numerique.gouv.fr/methode/glossaire/#contraste

Le calcul du contraste est donc une opération linéaire entre les deux couleurs en jeu. C'est une simplification qui ne correspond pas à la perception des contrastes par l'œil humain. Par conséquent, certaines combinaisons de couleurs qui sont conformes au critère de contraste sont quand même illisibles.
D'autres algorithmes existent pour essayer de coller à la perception humaine ([APCA étant le plus débattu en ligne](https://adrianroselli.com/2026/04/wcag3-contrast-as-of-april-2026.html)), et les WCAG 3 pourraient modifier la manière de calculer le contraste.

### État du web

[D'après WebAIM](https://webaim.org/projects/million/#contrast) qui étudie les non-conformités sur des millions de sites tous les ans, en 2026 83,9% des pages d'accueil des sites vérifiés échouaient à avoir un contraste suffisant entre le texte et l'arrière-plan.

# 2. Aucune information n'est donnée uniquement par la couleur
...
Si la couleur est la seule source d'information, les personnes ayant du mal à différencier certaines couleurs risquent de manquer cette information.

C'est par exemple le cas pour un champ de formulaire dont la bordure devient rouge (et rien d'autre) quand il est en erreur, ou pour les courbes d'un graphique qui sont uniquement différentiables par leur couleur.
